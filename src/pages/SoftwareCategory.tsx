...
import { softwareCategories, softwareByCategory } from "@/data/software";
import ProductCard from "@/components/cards/ProductCard";
import { branding } from "@/config/branding";

const SoftwareCategory = () => {
  const { slug } = useParams();
  const cat = softwareCategories.find((c) => c.slug === slug);
  const items = (slug && softwareByCategory[slug]) || [];
  ...
  return (
    <main className="container py-10">
      <Helmet>
        ...
        <meta name="description" content={`Explora software de ${cat.name}: ${cat.description}.`} />
        <link rel="canonical" href={`${branding.domain}/software/${cat.slug}`} />
      </Helmet>
